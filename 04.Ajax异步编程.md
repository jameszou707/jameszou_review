...
+ 概念：JSON(JavaScript Object Notation)，**JavaScript 对象表示法**
+ 作用：在计算机与网络之间存储和传输数据。
+ 本质： 就是字符串，表示 **对象和数组**
...
+ 注意：`key` 必须是使用英文的双引号包裹的字符串，`value` 的数据类型可以是**数字、字符串、**

  **布尔值、null、数组、对象**6种类型，如果是字符串，也必须用 **双引号**
...
+ 注意：类型可以是**数字、字符串、布尔值、null、数组、对象**6种类型，如果是字符串，也必须用 **双引号**
